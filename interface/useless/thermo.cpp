...
void Thermo::parse_fields(const std::string &str)
{
  ...
  while (keywords.has_next()) {
    ...
    // uncertainties
    } else if (word == "e_var") {                   // energy variance
      addfield("EngVar", &Thermo::compute_e_var,FLOAT);
    } else if (word == "e_sd") {                   // energy std
      addfield("EngStd", &Thermo::compute_e_sd,FLOAT);
    } else if (word == "f_var") {                   // force variance
      addfield("ForceVar", &Thermo::compute_f_var,FLOAT);
    } else if (word == "f_sd") {                   // force std
      addfield("ForceStd", &Thermo::compute_f_sd,FLOAT);
    } else {
      ArgInfo argi(word);
...
int Thermo::evaluate_keyword(const std::string &word, double *answer)
{
  ...
  // uncertainties
  else if (word == "e_var") compute_e_var();     
  else if (word == "e_sd") compute_e_sd();     
  else if (word == "f_var") compute_f_var();     
  else if (word == "f_sd") compute_f_sd();     
...
void Thermo::compute_e_var()
{
  dvalue = force->pair->e_var;
}

void Thermo::compute_e_sd()
{
  dvalue = force->pair->e_sd;
}

void Thermo::compute_f_var()
{
  dvalue = force->pair->f_var;
}

void Thermo::compute_f_sd()
{
  dvalue = force->pair->f_sd;
}