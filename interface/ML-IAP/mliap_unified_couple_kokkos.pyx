...
cdef extern from "pair.h" namespace "LAMMPS_NS":
    cdef cppclass Pair:
        pass


cdef extern from "pair_mliap.h" namespace "LAMMPS_NS":
    cdef cppclass PairMLIAP(Pair):
        void clear_uncertainties()
        void clear_uncertainty_arrays()
        void set_uncertainty(const string &, double)
        void set_uncertainty_array(const string &, const double *, int)
...
cdef class MLIAPDataPy:
    ...
    def clear_uncertainties(self):
        cdef PairMLIAP * pairmliap
        if self.data.pairmliap is NULL:
            return
        pairmliap = <PairMLIAP *> self.data.pairmliap
        pairmliap.clear_uncertainties()
    ...
    def clear_uncertainty_arrays(self):
        ...
        if self.data.pairmliap is NULL:
            return
        pairmliap = <PairMLIAP *> self.data.pairmliap
        pairmliap.clear_uncertainty_arrays()
    ...
    def set_uncertainty(self, key, value):
        ...
        pairmliap = <PairMLIAP *> self.data.pairmliap
        pairmliap.set_uncertainty(c_key, <double> value)

    def set_uncertainty_array(self, key, values):
        cdef string c_key
        cdef object values_arr
        cdef uintptr_t values_ptr
        cdef PairMLIAP * pairmliap
        if self.data.pairmliap is NULL:
            return
        values_arr = np.ascontiguousarray(values, dtype=np.float64).reshape(-1)
        values_ptr = <uintptr_t> values_arr.ctypes.data
        c_key = key.encode("utf-8")
        pairmliap = <PairMLIAP *> self.data.pairmliap
        pairmliap.set_uncertainty_array(
            c_key,
