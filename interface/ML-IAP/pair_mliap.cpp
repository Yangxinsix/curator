...
/* ----------------------------------------------------------------------
   scalar uncertainty requests
------------------------------------------------------------------------- */

void PairMLIAP::clear_uncertainties()
{
  for (auto &entry : uncertainty_scalars) entry.second = 0.0;
}

void PairMLIAP::set_uncertainty(const std::string &name, double value)
{
  uncertainty_scalars[name] = value;
}

double *PairMLIAP::ensure_uncertainty_ptr(const std::string &name)
{
  return &uncertainty_scalars[name];
}

double *PairMLIAP::get_uncertainty_ptr(const std::string &name)
{
  auto it = uncertainty_scalars.find(name);
  if (it == uncertainty_scalars.end()) return nullptr;
  return &it->second;
}

/* ----------------------------------------------------------------------
   per-atom uncertainty requests
------------------------------------------------------------------------- */

void PairMLIAP::clear_uncertainty_arrays()
{
  uncertainty_arrays.clear();
}

void PairMLIAP::set_uncertainty_array(const std::string &name, const double *values, int nvalues)
{
  if (values == nullptr || nvalues <= 0) {
    uncertainty_arrays.erase(name);
    return;
  }

  auto &target = uncertainty_arrays[name];
  target.assign(values, values + nvalues);
}

double *PairMLIAP::get_uncertainty_array_ptr(const std::string &name, int &nvalues)
{
  nvalues = 0;
  auto it = uncertainty_arrays.find(name);
  if (it == uncertainty_arrays.end()) return nullptr;
  if (it->second.empty()) return nullptr;
  nvalues = static_cast<int>(it->second.size());
  return it->second.data();
}

/* ----------------------------------------------------------------------
   expose uncertainty through Pair::extract()
------------------------------------------------------------------------- */

void *PairMLIAP::extract(const char *name, int &dim)
{
  dim = 0;

  int nvalues = 0;
  if (get_uncertainty_array_ptr(std::string(name), nvalues) != nullptr) return nullptr;

  double *value = get_uncertainty_ptr(std::string(name));
  if (value == nullptr) return nullptr;
  return static_cast<void *>(value);
}

void *PairMLIAP::extract_peratom(const char *name, int &ncol)
{
  ncol = 0;
  int nvalues = 0;
  double *values = get_uncertainty_array_ptr(std::string(name), nvalues);
