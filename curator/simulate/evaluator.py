...
def _extract_energy(atoms: Atoms) -> Optional[float]:
    if getattr(atoms, "info", None):
        for key in ("energy", "E"):
            if key in atoms.info:
                return _to_scalar(atoms.info[key])
    if getattr(atoms, "calc", None) is not None:
        try:
            return _to_scalar(atoms.get_potential_energy())
        except Exception:
            return None
    return None


def _extract_forces(atoms: Atoms) -> Optional[np.ndarray]:
    arrays = getattr(atoms, "arrays", None)
    if arrays and "forces" in arrays:
        return np.asarray(arrays["forces"])
    if getattr(atoms, "info", None) and "forces" in atoms.info:
        return np.asarray(atoms.info["forces"])
    if getattr(atoms, "calc", None) is not None:
        try:
            return np.asarray(atoms.get_forces(apply_constraint=False))
        except Exception:
            return None
    return None


def _atoms_iter(datapath: Union[str, Sequence[str], Iterable[Atoms]]) -> Iterable[Atoms]:
    if isinstance(datapath, (list, tuple)):
        if datapath and all(isinstance(item, Atoms) for item in datapath):
            return datapath
    if isinstance(datapath, str) and datapath.startswith(("http://", "https://")):
        return _prepare_data_source(datapath)
    return read_trajectory(datapath)
...
class Evaluator:
    ...
    def _resolve_output_dir(self, datapath: Union[str, Sequence[str], Iterable[Atoms]]) -> Path:
        ...
        if isinstance(datapath, (str, Path)):
            ...
            if len(datapath) == 1:
                tag = Path(datapath[0]).stem or "dataset"
            else:
                tag = "multi_dataset"
    ...
    def _plot_results(
        ...
        if energy_true:
            ...
            if "energy" in metrics:
                text = f"MAE={metrics['energy']['mae']:.4g}\nRMSE={metrics['energy']['rmse']:.4g}"
        ...
        if forces_true:
            ...
            if "forces" in metrics:
                text = f"MAE={metrics['forces']['mae']:.4g}\nRMSE={metrics['forces']['rmse']:.4g}"
                ax.text(0.02, 0.98, text, transform=ax.transAxes, va="top")
            fig.tight_layout()
