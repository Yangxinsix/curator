...
def NEB(config: DictConfig,MLcalc: Calculator,PE: PrintEnergy) -> None:
    ...
    neb.interpolate(mic=config.mic)
    ...
    # Set up NEB control
    printenergy = NEB_control(PE)
    optimizer.attach(printenergy, interval=config.print_step)
    log.info('run NEB')
